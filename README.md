...
- Use **Save & load → Download backup (.json)** every so often to keep an external copy, and **Restore backup (.json)** to bring it back on another browser or device. On desktop Chrome/Edge, the **Use a file** option (offered near the top of the page, or in the status bar at the bottom) connects a local file that autosaves continuously instead.
